...
namespace SIPSorcery.Net
{
    ...
    public class RTPSession : IMediaSession, IDisposable
    {
        ...
        private void OnReceiveRTCPPacket(int localPort, IPEndPoint remoteEndPoint, ReadOnlySpan<byte> buffer)
        {
            ...
            byte[] unprotectedBuffer = null;

            if (secureContext != null)
            {
                unprotectedBuffer = buffer.ToArray();
                int res = secureContext.UnprotectRtcpPacket(unprotectedBuffer, unprotectedBuffer.Length, out int outBufLen);
                if (res != 0)
                {
                ...
                {
                    buffer = unprotectedBuffer.AsSpan(0, outBufLen);
                }
            }
        ...
        private void OnReceiveRTPPacket(int localPort, IPEndPoint remoteEndPoint, ReadOnlySpan<byte> buffer)
        {
        ...
        private MediaStream GetMediaStream(uint ssrc)
        {
            ...
            else if (VideoStreamList.Count == 1)
            {
                if (HasVideo)
                {
