...
namespace SIPSorcery.Net
{
    public class MediaStream
    {
        ...
        public bool EnsureBufferUnprotected(ReadOnlySpan<byte> buf, RTPHeader header, out RTPPacket packet)
        {
            ...
            {
                var (succeeded, newBuffer) = UnprotectBuffer(buf.ToArray());
                if (!succeeded)
                {
                    packet = null;
                    return false;
        ...
        public void OnReceiveRTPPacket(RTPHeader hdr, int localPort, IPEndPoint remoteEndPoint, ReadOnlySpan<byte> buffer)
        {
            RTPPacket rtpPacket;
        ...
        protected virtual bool AddPendingPackage(RTPHeader hdr, int localPort, IPEndPoint remoteEndPoint, ReadOnlySpan<byte> buffer)
        {
            ...
            if (SecureContext == null && !IsClosed)
            {
                lock (_pendingPackagesLock)
                {
                    ...
                    _pendingPackagesBuffer.Add(new PendingPackages(hdr, localPort, remoteEndPoint, buffer.ToArray()));
                }
                return true;
            }
            return false;
