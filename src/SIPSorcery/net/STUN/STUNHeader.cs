...
namespace SIPSorcery.Net
{
    ...
    public class STUNHeader
    {
        ...
        {
            return ParseSTUNHeader(bufferSegment.Array.AsSpan(bufferSegment.Offset, bufferSegment.Count));
        }
        ...
        public static STUNHeader ParseSTUNHeader(ReadOnlySpan<byte> bufferSegment)
        {
            ...
            if (bufferSegment.Length > 0 && bufferSegment.Length >= STUN_HEADER_LENGTH)
            {
                ...
                UInt16 stunTypeValue = BinaryPrimitives.ReadUInt16BigEndian(bufferSegment);
                UInt16 stunMessageLength = BinaryPrimitives.ReadUInt16BigEndian(bufferSegment.Slice(2));

                stunHeader.MessageType = STUNMessageTypes.GetSTUNMessageTypeForId(stunTypeValue);
                stunHeader.MessageLength = stunMessageLength;
                bufferSegment.Slice(8, TRANSACTION_ID_LENGTH).CopyTo(stunHeader.TransactionId);
