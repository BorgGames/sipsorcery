...
namespace SIPSorcery.Net
{
    ...
    public class RTCSctpTransport : SctpTransport
    {
        ...
        public void Start(DatagramTransport dtlsTransport, bool isDtlsClient)
        {
            ...
            {
                logger.LogWarning("RTCSctpTransport for association {ID} has already been started.", RTCSctpAssociation.ID);
            }
        }
        ...
        private void DoReceive(object state)
        {
            ...
            while (!_isClosed.HasOccurred)
            {
                try
                {
                    ...
                    {
                        ReadOnlySpan<byte> packetBuffer = recvBuffer.Slice(0, bytesRead);

                        if (!SctpPacket.VerifyChecksum(packetBuffer))
                        {
                            logger.LogWarning("SCTP packet received on DTLS transport dropped due to invalid checksum.");
                        }
                        else
                        {
                            var pkt = SctpPacketView.Parse(packetBuffer);
        ...
        public override void Send(string associationID, ReadOnlySpan<byte> buffer)
        {
            ...
            if (!_isClosed.HasOccurred)
            {
                lock (transport)
                {
                    ...
                    {
                        buffer.CopyTo(tmp);
                        transport.Send(tmp, 0, buffer.Length);
                    }
                    finally
                    {
